...
from fintts_postbank.config import get_xmpp_settings
...
async def run_xmpp_mode_async(force_tan_selection: bool = False) -> None:
    ...
    manager = XmppSessionManager(bot, event_loop, force_tan_selection)
...
def run_xmpp_mode(force_tan_selection: bool = False) -> None:
    """Run the FinTS client in XMPP bot mode.

    Args:
        force_tan_selection: Whether to force TAN mechanism selection
    """
    asyncio.run(run_xmpp_mode_async(force_tan_selection))
